...
const state = {
  version: "TIERRA-ADAPTIVE-BAYES-KELLY-FIB-v3.1-ENTRY-FIX",
...
async function scan(){
  ...
  try{
    ...
    if(!LIVE){
      ...
      const chosen=candidates.find(c=>sideCount(c.side)<CFG.maxSameSide) || null;
    ...
    state.execution.eligibleCandidates=candidates.length;state.execution.botEnabled=state.botEnabled;state.execution.bestCandidate=candidates[0]?{symbol:candidates[0].symbol,side:candidates[0].side,probability:+(candidates[0].probability*100).toFixed(1),edgePct:+candidates[0].edgePct.toFixed(2),score:+candidates[0].score.toFixed(2)}:null;
