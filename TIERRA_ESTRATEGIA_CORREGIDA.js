...
function bestCandidates(results, liquidityMap) {
  ...
  for (const x of results) {
    ...
    for (const side of ["LONG", "SHORT"]) {
      // First pass: technical + lead/lag + liquidity only.
      // Microstructure is deliberately NOT trusted until enrichMicro() completes.
      const tech = side === "LONG" ? x.longTech : x.shortTech;
      const lead = leadLagScore(x, side);
      const qv = liq?.quoteVol || 0;
      const liqScore = qv >= 5e6 ? .8 : qv >= 1e6 ? .4 : qv >= 2e5 ? 0 : -.8;
      const preScore = tech + lead + liqScore;
      arr.push({ ...x, side, preScore, quoteVol:qv, change24:liq?.change24||0 });
    }
  }
...
function managePosition(p, x) {
  ...
  let reason = null;
  const risk = Math.abs(p.entry - (p.side === "LONG" ? p.stop : p.stop));
  const favorable = p.side === "LONG" ? x.price - p.entry : p.entry - x.price;

  // Protect a trade that has already moved in our favor instead of allowing a
  // winner to return all the way to the original stop.
  if (favorable >= risk * 0.75) {
    const be = p.entry * (p.side === "LONG" ? 1.00035 : 0.99965);
    p.stop = p.side === "LONG" ? Math.max(p.stop, be) : Math.min(p.stop, be);
  }
  if (favorable >= risk * 1.35) {
    const trail = Math.max(x.atr * 0.9, x.price * 0.0025);
    const t = p.side === "LONG" ? x.price - trail : x.price + trail;
    p.stop = p.side === "LONG" ? Math.max(p.stop, t) : Math.min(p.stop, t);
  }

  if (p.side === "LONG") {
    if (x.price <= p.stop) reason = "STOP_OR_TRAIL";
    else if (x.price >= p.tp) reason = "TP";
    else if (x.regime === "BEAR" && x.ema20 < x.ema50 && favorable <= 0) reason = "REGIME_FLIP";
  } else {
    if (x.price >= p.stop) reason = "STOP_OR_TRAIL";
    else if (x.price <= p.tp) reason = "TP";
    else if (x.regime === "BULL" && x.ema20 > x.ema50 && favorable <= 0) reason = "REGIME_FLIP";
  }
...
async function scan() {
  ...
  try {
    ...
    // CRITICAL FIX: enrich the strongest technical candidates BEFORE calculating
    // the final score. The previous version calculated the score first, when OI,
    // funding, order-book and taker-flow were still null. That meant the dashboard
    // displayed microstructure fields but the entry decision largely ignored them.
    const enrichTargets = candidates.slice(0, 48);
    const enriched = [];
    ...
    for (const c of candidates) {
      ...
      if (!e || !e.microOk) continue;
      const liq = liquidityMap.get(c.symbol);
      const final = finalCandidate(e, c.side, liq);
      // Require agreement from at least one live microstructure source.
      // This prevents a purely technical score from opening a trade against flow.
      const flow = final.microScore;
      const aligned = flow >= 0.8;
      if (aligned && final.score >= CFG.minEdge) rescored.push(final);
    }
    candidates = rescored.sort((a, b) => b.score - a.score);
