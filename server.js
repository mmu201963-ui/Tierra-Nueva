...
app.post("/api/close/:symbol", (req, res) => {
...
async function closeOne(encoded){const symbol=decodeURIComponent(encoded);if(!confirm('¿Cerrar '+symbol+'?'))return;try{const r=await fetch('/api/close/'+encodeURIComponent(symbol),{method:'POST'});const j=await r.json();if(!j.ok)alert(j.error||'No se pudo cerrar');load()}catch(e){alert(e.message)}}
