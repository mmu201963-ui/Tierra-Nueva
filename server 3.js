...
async function scan() {
  ...
  try {
    ...
    // Escaneo rotativo: analiza una parte del mercado por ciclo y conserva
    // los resultados anteriores. Así el bot puede repetir ciclos rápidamente
    // sin disparar cientos de peticiones simultáneas a Binance.
    const start = Number(state.scanCursor || 0) % symbols.length;
    const batch = [];
    for (let n = 0; n < Math.min(CFG.analysisBatch, symbols.length); n++) {
      batch.push(symbols[(start + n) % symbols.length]);
    }
    state.scanCursor = (start + batch.length) % symbols.length;
    state.currentBatch = batch.length;
    state.batchStart = start;

    const out = await Promise.all(batch.map(async symbol => {
      try {
        return await analyzeSymbol(symbol);
      } catch (e) {
        state.lastError = `ANALYZE ${symbol}: ${e.message}`;
        logEvent("ANALYZE_ERROR", symbol, { message: e.message });
        return null;
      }
    }));
    for (const x of out) {
      if (x) state.analysisCache[x.symbol] = x;
    }
    const results = Object.values(state.analysisCache);
    state.dataMarkets = results.length;
    state.marketProgress = `${Math.min(results.length, symbols.length)}/${symbols.length}`;
...
app.post("/api/start", (_q,res)=>{
  ...
  res.json({ok:true,mode:state.mode,status:state.status});
  if(!state.scanRunning) scan().catch(e=>{state.lastError=e.message;logEvent("START_SCAN_ERROR",null,{message:e.message});});
});
...
async function load(){try{const r=await fetch('/status?ts='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error('/status HTTP '+r.status);const s=await r.json();let h='<div class="grid">';h+='<div class="card"><div class="muted">ESTADO</div><div class="big">'+esc(s.status)+'</div><div>Scan '+s.scans+' · mercados '+s.dataMarkets+'/'+s.markets+' · lote '+(s.currentBatch||0)+' · '+s.scanMs+' ms</div><div class="muted">Progreso acumulado: '+esc(s.marketProgress||'0/0')+' · cursor '+(s.scanCursor||0)+'</div>' + (s.lastError?'<div class="bad">Error: '+esc(s.lastError)+'</div>':'')</div>';h+='<div class="card"><div class="muted">EQUITY</div><div class="big">$'+Number(s.equity).toFixed(2)+'</div><div>Realizado $'+Number(s.realizedPnl).toFixed(2)+' · flotante $'+Number(s.floatingPnl).toFixed(2)+'</div></div>';h+='<div class="card"><div class="muted">BTC CONTEXTO</div><div>'+['1m','5m','15m','1h'].map(tf=>'<span class="tag">'+tf+': '+esc(s.marketContext?.[tf]||'—')+'</span>').join('')+'</div></div></div>';
